...
import { MatchRadarChart } from "@/components/radar-chart";
  ...
  const { data: job, isLoading: jobLoading } = useQuery<Job>({
    ...
    queryFn: async () => {
      try {
        ...
        // Mock data
        return {
          id: 1, job_id: jobId, apply_url: '#', title_en: 'Senior Backend Engineer', 
          company_name_en: 'TechCorp', job_address: 'Ho Chi Minh', salary_raw: '20M-30M VND', 
          experience_min_years: 1.5, experience_max_years: 3, job_type: 'Full-time',
          job_requirements_en: '- Python\n- FastAPI\n- PostgreSQL\n- Docker', 
          job_description_en: 'We are looking for a backend engineer...\n\nResponsibilities:\n- Build APIs',
          benefit_en: '- 13th month salary\n- Health insurance',
          is_active: true
        } as Job;
      }
    }
  ...
  const { data: match, isLoading: matchLoading } = useQuery<MatchResult>({
    ...
    queryFn: async () => {
      try {
        ...
        return {
          rank: 1, job_id: jobId, apply_url: '#', 
          scores: { overall: 0.87, hgat_cosine: 0.85, skill_match: 0.92, experience_match: 0.85, salary_match: 0.78, location_match: 1.0, slwg_total_penalty: 0.7 },
          skill_analysis: {
            matched_skills: ['Python', 'FastAPI', 'PostgreSQL'],
            missing_required: [{ skill: 'Docker', tier: 'hard', omega: 0.7, slwg_penalty: 0.7, suggestion: 'Take a Docker course' }],
            missing_preferred: []
          }
        } as MatchResult;
      }
        ...
        <div className="w-full lg:w-[60%] space-y-8">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-6 text-sm text-primary/80">
              <span className="flex items-center gap-2"><MapPin className="w-4 h-4 text-muted" /> {job.job_address || 'Remote'}</span>
              <span className="flex items-center gap-2"><DollarSign className="w-4 h-4 text-muted" /> {job.salary_raw || 'Negotiable'}</span>
              <span className="flex items-center gap-2"><Briefcase className="w-4 h-4 text-muted" /> {job.job_type || 'Full-time'}</span>
              {job.experience_min_years !== undefined && (
                <span className="flex items-center gap-2"><Calendar className="w-4 h-4 text-muted" /> {job.experience_min_years} - {job.experience_max_years} years</span>
              )}
            </div>
          </div>
          ...
          {requirements.length > 0 && (
            <div>
              <h3 className="font-fraunces text-xl font-bold mb-4">Requirements</h3>
          ...
          {/* Full Description */}
          <div>
            <h3 className="font-fraunces text-xl font-bold mb-4">Job Description</h3>
            <div className="prose prose-invert max-w-none text-muted leading-relaxed whitespace-pre-wrap font-dm-sans">
              {job.job_description_en || job.job_description_vi || 'No description provided.'}
            </div>
          </div>
          ...
          {benefits.length > 0 && (
            <div>
              <h3 className="font-fraunces text-xl font-bold mb-4">Benefits</h3>
        ...
        <div className="w-full lg:w-[40%] sticky top-24">
          {status !== 'authenticated' ? (
            <div className="bg-surface border border-border p-8 rounded-xl text-center">
              ...
              <h3 className="font-fraunces text-xl font-bold mb-2">Sign in to see your match</h3>
              <p className="text-muted text-sm mb-6">Our AI will analyze your CV against this job's requirements to predict your success.</p>
            ...
            <div className="bg-surface border border-border rounded-xl overflow-hidden shadow-2xl">
              <div className="p-6 border-b border-border bg-base/50 text-center relative overflow-hidden">
                ...
                <h3 className="font-fraunces text-lg font-bold text-primary relative z-10">AI Match Analysis</h3>
                <div className="font-jetbrains-mono text-5xl font-bold text-accent mt-2 relative z-10">
                  {Math.round(match.scores.overall * 100)}%
                </div>
              </div>
              ...
              <div className="p-6">
                ...
                <h4 className="font-medium text-sm text-primary mb-4 uppercase tracking-widest">SLWG Gap Analysis</h4>
                ...
                {match.skill_analysis.missing_required.length === 0 ? (
                  <div className="text-sm text-success bg-success/10 p-3 rounded-md border border-success/20">
                    No critical skill gaps detected! You meet all core requirements.
                  </div>
                ) : (
