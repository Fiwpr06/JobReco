...
import { Sparkles, Upload, BrainCircuit, Brain, FileSearch, BriefcaseBusiness, TrendingUp, ArrowRight } from 'lucide-react';
...
export default function LandingPage() {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/api/v1/skills/trends').catch(() => ({ data: [
        { skill_name: 'ReactJS', demand_score: 95 },
        { skill_name: 'Next.js', demand_score: 92 },
        { skill_name: 'NodeJS', demand_score: 90 },
      ]})),
      api.get('/api/v1/jobs?limit=3').catch(() => ({ data: [
    ...
    <div className="flex-1 bg-base text-primary overflow-x-hidden">
        ...
        <Container className="grid min-h-[80vh] items-center gap-12 lg:grid-cols-2">
          ...
          >
            ...
            <p className="mt-8 max-w-2xl text-lg leading-8 text-muted mx-auto lg:mx-0">
              Upload CV và để JobReco phân tích kỹ năng, kinh nghiệm và định hướng nghề nghiệp dựa trên mạng thần kinh Graph neural network để gợi ý những công việc tốt nhất cho bạn.
            </p>

            <div className="mt-10 flex flex-col gap-4 sm:flex-row justify-center lg:justify-start">
              <Link href="/cv">
                <button className="w-full sm:w-auto flex items-center justify-center gap-2 rounded-2xl border-2 border-indigo-500 bg-indigo-600 px-8 py-4 text-lg font-black text-white shadow-[0_0_20px_var(--accent-glow)] transition hover:scale-105 hover:bg-indigo-500 cursor-pointer">
                  <Upload size={20} />
                  Upload CV
                </button>
              </Link>
          ...
          >
            <div className="rounded-[32px] border border-border bg-surface/50 p-8 shadow-2xl backdrop-blur-xl">
              <div className="mb-8 flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-primary">AI Job Analysis</h3>
                  <p className="text-muted">Phân tích CV thông minh</p>
                </div>
              </div>

              <div className="space-y-6">
                {demoJobs.length > 0 ? demoJobs.map((job, idx) => {
                  const mockScores = [92, 89, 75];
                  const score = mockScores[idx] || 70;
                  ...
                  return (
                    <div key={idx}>
                      <div className="mb-2 flex justify-between">
                        <span className="font-medium text-primary line-clamp-1 pr-4">{title}</span>
                        <span className={`font-bold ${color}`}>{score}%</span>
                      </div>
                      <div className="h-3 rounded-full bg-elevated">
                        <div className="h-3 rounded-full bg-gradient-to-r from-indigo-500 to-cyan-500" style={{ width: `${score}%` }}></div>
                      </div>
                    </div>
                  );
                }) : (
                  [1, 2, 3].map(i => (
                     <div key={i} className="animate-pulse">
                        <div className="mb-2 flex justify-between">
                          <div className="h-4 w-32 bg-elevated rounded"></div>
                          <div className="h-4 w-8 bg-elevated rounded"></div>
                        </div>
                        <div className="h-3 rounded-full bg-elevated w-full"></div>
                     </div>
                  ))
                )}
              </div>
              ...
              <div className="mt-8 rounded-2xl bg-elevated p-5 border border-border-mid">
                <h4 className="font-bold text-primary">Kỹ năng còn thiếu</h4>

                <div className="mt-4 flex flex-wrap gap-3">
                  <span className="rounded-full bg-danger/10 border border-danger/20 px-4 py-2 text-sm font-medium text-danger">
                    Docker
                  </span>
        ...
        <Container>
          <div className="grid gap-6 md:grid-cols-4">
            {[
              ...
              { value: '24/7', label: 'AI hoạt động liên tục' }
            ].map((item, idx) => (
              <motion.div
                key={idx}
                whileHover={{ y: -5 }}
      ...
      <section className="py-12 border-b border-border bg-base/50">
        <div className="container mx-auto px-6 mb-6">
          <h3 className="text-sm font-medium text-muted uppercase tracking-widest text-center lg:text-left">Kỹ năng đang hot trong tuần</h3>
        </div>
        <div className="flex gap-4 px-6 overflow-x-auto pb-4 hide-scrollbar">
          {loading ? (
            [1, 2, 3, 4, 5].map(i => (
              <div key={i} className="h-12 w-36 bg-elevated rounded-full animate-pulse flex-shrink-0" />
            ))
          ) : (
            trends.map((trend, i) => (
              <div key={i} className="flex-shrink-0 px-5 py-3 rounded-full border border-border-mid bg-surface flex items-center gap-3 hover:border-indigo-500/50 transition-colors">
                <span className="font-jetbrains-mono font-medium text-primary">{trend.skill_name}</span>
                <span className="w-2 h-2 rounded-full bg-success" />
                <span className="text-xs text-muted">Demand: {trend.demand_score}%</span>
              </div>
            ))
          )}
        </div>
        ...
        <Container>
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-6 text-lg leading-8 text-muted">
              JobReco giúp ứng viên tìm việc nhanh hơn và doanh nghiệp tìm đúng người phù hợp nhất dựa trên năng lực.
            </p>
          </div>
            ...
            {[
              {
                icon: Brain,
                title: 'AI Matching',
                description: 'Thuật toán Graph neural network so khớp kỹ năng và học vấn của bạn để lọc ra việc tương thích.'
              },
              {
                icon: FileSearch,
                title: 'Phân tích CV',
                description: 'Tự động trích xuất thông tin, kỹ năng, kinh nghiệm từ CV PDF nhanh chóng và chính xác.'
              },
              {
                icon: BriefcaseBusiness,
                title: 'Gợi ý việc làm',
                description: 'Bảng tin đề xuất được cá nhân hóa và xếp hạng theo điểm số tương thích thực tế.'
              },
              {
                icon: TrendingUp,
                title: 'Skill Gap Analysis',
                description: 'Hiển thị trực quan những kỹ năng còn thiếu kèm theo lộ trình học tập tối ưu nhất.'
              }
            ].map((feature, idx) => {
        ...
        <Container>
          <div className="text-center">
            ...
            <p className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-muted">
              Chỉ với vài bước đơn giản, AI thông minh của chúng tôi sẽ giúp bạn mở rộng cơ hội nghề nghiệp.
            </p>
          </div>
            ...
            {[
              {
                step: '01',
                title: 'Upload CV',
                description: 'Tải CV PDF/DOCX hiện có của bạn lên hệ thống chỉ với một cú nhấp chuột.'
              },
              {
                step: '02',
                title: 'AI Phân Tích',
                description: 'Mô hình học máy phân tích từ khóa, sơ đồ kỹ năng và độ phủ kinh nghiệm thực tế.'
              },
              {
                step: '03',
                title: 'Nhận Đề Xuất',
                description: 'Nhận ngay danh sách công việc đề xuất tối ưu cùng các kỹ năng cần bổ sung.'
              }
            ].map((step, idx) => (
              <div
                key={idx}
        ...
        <Container>
          <div className="text-center">
            ...
            <p className="mt-6 text-lg text-muted">
              Xem ứng viên và doanh nghiệp nói gì về hiệu quả kết nối từ JobReco.
            </p>
          </div>
            ...
            {[
              {
                ...
                content: 'JobReco giúp tôi khớp chuẩn các kỹ năng React và Next.js. Tìm được job ưng ý chỉ sau 3 ngày upload CV!'
              },
              {
                name: 'Nguyễn Thị Hương',
                role: 'Data Analyst',
                content: 'Khả năng chỉ ra Skill Gap cực kỳ đáng giá. Nhờ hệ thống mà tôi biết mình cần bổ sung thêm kỹ năng Docker và SQL nâng cao.'
              },
              {
                name: 'Lê Hoàng Nam',
                role: 'Tech Lead @ KMS',
                content: 'Nền tảng giúp lọc đúng ứng viên có năng lực tương thích cao, giảm thiểu đáng kể thời gian phỏng vấn và đào tạo.'
              }
            ].map((item, idx) => (
              <div
                key={idx}
              ...
              <div className="flex flex-col items-center gap-10 md:flex-row">
                <div className="w-full md:w-3/4 text-center md:text-left">
                  ...
                  <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-300">
                    Upload CV ngay hôm nay để trí tuệ nhân tạo của chúng tôi giúp bạn mở khóa những cơ hội nghề nghiệp tốt nhất thị trường.
                  </p>
                </div>
